...
export function AddAnnotationRecord(props: AddRecordProps) {
    const [userName, setUserName] = React.useState<string>(
        props.allUsers[0].id,
    );
    const [annotationStatus, setAnnotationStatus] = React.useState<string>(
        ANNOTATION_STATUS_OPTIONS[0].id,
    );
    ...
    async function addButtonPressed() {
        ...
        const addRecordCall = await callApi(
            annotationFields,
            API_ROUTES.ADD_ANNOTATION,
            API_METHODS.POST,
        );
        if (addRecordCall.statusCode !== 200) {
            setError(
                `Issue with API call, failed with message: ${addRecordCall.body}`,
            );
